...
function submitForm() {
  ...
  if (formType === 2) {
    if (
      document.querySelector("#singleChoiceInput").value >= 1 &&
      document.querySelector("#singleChoiceInput").value <= 4
    ) {
      document.querySelector(".submit-button").type = "submit";
      document.querySelector(".submit-button").click();
    }
  ...
  if (formType === 3) {
    if (
      document.querySelector("#InputMultiple1").value == 1 ||
      document.querySelector("#InputMultiple2").value == 1 ||
      document.querySelector("#InputMultiple3").value == 1 ||
      document.querySelector("#InputMultiple4").value == 1
    ) {
      document.querySelector(".submit-button").type = "submit";
      document.querySelector(".submit-button").click();
    }
...
function singleChoice(button, value) {
  document.querySelector("input-singleChoice");
  document
    .querySelectorAll("button")
    .forEach(button => button.classList.remove("buttonInput-active"));
...
function addInputs(responseData) {
  ...
  if (!responseData.input_type && !responseData.singleOption) {
    ...
    document.querySelector(".normalForm-input").innerHTML = `
        ...
        <button type="button" onclick="multipleChoice(this,'2InputMultiple')" >${
          responseData.option2
        }</button>
        <button type="button" onclick="multipleChoice(this,'3InputMultiple')" >${
          responseData.option3
        }</button>
        <button type="button" onclick="multipleChoice(this,'4InputMultiple')" >${
          responseData.option4
        }</button>
      </div>
        `;
  }
